...
let Grolem=new Pokemon('Grolem',14,8,100,0.5);
let Pikachu=new Pokemon('Pikachu',18,4,70,0.8);

while(Grolem.hp >0 && Pikachu.hp>0){
    Grolem.attackPokemon(Pikachu)
    if(Pikachu.hp<=0){
        console.log(Pikachu.name+" is dead !");
        break;
    }
    Pikachuss .attackPokemon(Grolem)
    if(Grolem.hp<=0){
        console.log(Grolem.name+" is dead !");
        break
    }
}